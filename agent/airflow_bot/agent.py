...
model_name = os.getenv("AIRFLOW_AGENT_MODEL", "gemini-3-pro-preview")
...
generate_content_config = types.GenerateContentConfig(
    ...
    # Increase max_output_tokens to handle detailed DAG run history, task logs, and batch operations
    # Default to 16384 (16k tokens) for complex Airflow operations, can be overridden via env var
    max_output_tokens=_env_int("AIRFLOW_AGENT_MAX_OUTPUT_TOKENS", 16384),
)
...
async def _log_agent_response(callback_context):
    ...
    try:
        ...
        if hasattr(callback_context, "response"):
            ...
            logger.info("airflow_bot response received, type=%s", type(response).__name__)
            
            if hasattr(response, "text"):
                text_len = len(response.text) if response.text else 0
                estimated_tokens = text_len / chars_per_token
                logger.info("airflow_bot response text length=%d chars (~%d tokens, max=%d)", 
                           text_len, int(estimated_tokens), max_tokens)
                
                if estimated_tokens >= max_tokens * 0.9:
                    logger.warning("airflow_bot response may be approaching token limit! "
                ...
                if response.text:
                    ...
                    if any(indicator in response.text[-100:].lower() for indicator in truncation_indicators):
                        logger.warning("airflow_bot response may be truncated!")
    except Exception as e:
        logger.exception("Failed to log airflow_bot response: %s", e)
    return None
