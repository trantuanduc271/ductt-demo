...
ROOT_SYSTEM_INSTRUCTION = (
    ...
    "   - Deletes users and all associated records from the database\n"
    ...
    "- Use awareness_agent for: gap detection, assignment planning, approvals, data import, user deletion\n"
    ...
    "USER DELETION:\n"
    "When a user requests to delete a user (e.g., 'delete user jake' or 'remove user u-007'):\n"
    "1. Route the request to awareness_agent (NOT airflow_assistant or kubernetes_assistant)\n"
    "2. The awareness_agent has a delete_user tool that will permanently remove the user and all associated records\n"
    "3. This is a destructive operation - the awareness_agent will handle the deletion\n\n"
    "JSON DATA IMPORT:\n"
    ...
    "- Deleting Kubernetes resources (high risk)\n"
    "- Modifying configurations (medium risk)\n"
    "- Restarting services (medium risk)\n"
    "- Scaling operations (low-medium risk)\n"
    "- Note: User deletion from the database is handled by awareness_agent directly (it manages its own approval workflow)\n\n"
...
awareness_remote_agent = RemoteA2aAgent(
    ...
    description=(
        ...
        "importing user awareness status from JSON data into the database, and deleting users from the system."
    ),
