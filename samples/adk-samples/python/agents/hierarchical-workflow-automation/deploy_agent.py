...
def deploy_cookie_agent():
    ...
    try:
        # Import required libraries for Vertex AI Agent Engine
        import vertexai
        ...
        if staging_bucket_env:
            staging_bucket = f"gs://{staging_bucket_env}" if not staging_bucket_env.startswith("gs://") else staging_bucket_env
        else:
        ...
        logger.info("   This may take several minutes as containers are built and started...")
        
        return remote_agent
        
    except ImportError as e:
        logger.error(f"Import Error!!!!: {e}")
        logger.error("Make sure google-adk is installed and the agent module is in the Python path")
        raise
